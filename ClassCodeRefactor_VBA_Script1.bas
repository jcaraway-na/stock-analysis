...
Dim worksheetExist As Boolean
    ...
    If worksheetExist = True Then
        ...
        'dynamically counts rows to apply conditional formatting to.
        'no need for dataRowStart & dataRowEnd variables.
        'user can add tickers to column A without having to go into code behind
        'to change cell row reference.
        ...
        For i = 1 To rowCount
            ...
            'set row start
            r = Range("A3").Offset(i, 0).Row
            
            If Range("C" & r).Value > 0 Then
                        
                Range("C" & r).Interior.Color = vbGreen
            
            ElseIf Range("C" & r).Value < 0 Then
                
                Range("C" & r).Interior.Color = vbRed
            
            Else
                    Range("C" & r).Interior.Color = xlNone
            End If
